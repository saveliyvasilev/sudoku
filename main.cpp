...
static void show_usage(std::string name)
{
    std::cerr << "Usage: " << name << " <option>[argument]"
              ...
              << "\t--pdf <npages> <complexity> \tGenerates a pdf with npages pages with 6 sudokus each, with specified complexity"
              << std::endl;
}
...
int main(int argc, char **argv){
	...
	if(opt == "--pdf"){
		...
		SudokuToLatex stl = SudokuToLatex("./output.tex", atoi(argv[2]), atof(argv[3]));
		stl.execute();
	}
}