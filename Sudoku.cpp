...
void Sudoku::unset(int i, int j) { set(i, j, 0); }
...
void Sudoku::reset_sq_non_candidates(int sq_i, int sq_j,
                                     std::bitset<10> &cs) const {
  for (int i = sq_i * 3; i < sq_i * 3 + 3; i++)
    for (int j = sq_j * 3; j < sq_j * 3 + 3; j++)
      cs.reset(m[i][j]);
}

int Sudoku::get_value(int i, int j) const { return m[i][j]; }
