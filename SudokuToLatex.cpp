...
SudokuToLatex::SudokuToLatex(std::string file, unsigned int npages,
                             double complexity) {
  this->file = file;
...
void SudokuToLatex::execute() {
  ...
  for (unsigned int i = 0; i < npages; i++) {
    ...
    for (unsigned int j = 0; j < 6; j++) {
      ...
      sudoku_body(out, gen);
      sudoku_footer(out, std::to_string(1 + j) + " - Complexity: " +
                             std::to_string(actual_complexity));
    }
    page_footer(out);
  }
  for (unsigned int i = 0; i < npages; i++) {
    ...
    for (unsigned int j = 0; j < 6; j++) {
      ...
      sudoku_footer(out, "Page " + std::to_string(1 + i) + " - " +
                             std::to_string(1 + j));
    }
    page_footer(out);
  }
