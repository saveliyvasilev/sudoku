...
#define POSSIBLE_RNODES 100	// Bound on how many recursive nodes to visit before giving up the execution
...
bool Generator::complexify_rec(Sudoku &s, const double &complexity, unsigned long &visited_nodes){
	Solver solver;
	visited_nodes++;

	if(visited_nodes > POSSIBLE_RNODES) return false;
...
std::tuple<Sudoku, Sudoku, double> Generator::generate(const double complexity){
	int cur;
	Solver solver;
	Sudoku generated;
	for(int k = 1; k < 9; k++)
